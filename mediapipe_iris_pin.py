...
def draw_eye_iris(
    ...
    big_x = iris[0][0][0] - iris[1][0][0]
    big_y = iris[0][0][1] - iris[1][0][1]
    big = (big_x**2 + big_y**2)**(1/2)
    
    pin_d = int(round(radius*4000/big, 0))

    c_row = iris[1][0][1]
    b_col = iris[1][3][0]
    e_col = iris[1][1][0]

    sector = img[c_row][b_col+3:b_col+23]

    rgb = np.argwhere(sector == 255)
    sector = np.delete(sector, rgb[:,0], axis = 0)
    
    pin_c = (np.sum(sector, axis = 0).argmax() + 1)*1000

    pin = pin_c + pin_d

    print(pin)

    name = sys.argv[2][-12:-4]
    print(name)
    ...
    with open("slike_nenad_iris//pins.txt", 'a+') as f:
        f.write(': PIN - ' + str(pin) + '\n')
    '''
